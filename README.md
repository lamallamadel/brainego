...
## Phase 1 MVP Local Setup (one-command startup)

This section documents the **minimum local setup** to run the Phase 1 stack in one command.

### Hardware prerequisites (recommended)

- **CPU:** 8+ vCPU (x86_64)
- **RAM:** 16 GB minimum (32 GB recommended)
- **Disk:** 25 GB free (images, volumes, logs, model file)
- **GPU:** NVIDIA GPU with at least 8 GB VRAM (for local LLM inference performance)
- **OS:** Linux, macOS, or Windows with Docker Desktop / Docker Engine + Compose v2

> If no GPU is available, infrastructure services still start, but model inference throughput/latency may degrade significantly.

### Environment variables (Phase 1)

Create a `.env` file at the repository root to override defaults used by `docker-compose.phase1.yaml`.
...
MAX_SERVE_MAX_BATCH_SIZE=32
MAX_SERVE_MAX_WAITING_TIME_MS=10
MAX_SERVE_MAX_TOKENS=2048
...
# Postgres
POSTGRES_PORT=5432
POSTGRES_DB=brainego
POSTGRES_USER=brainego
POSTGRES_PASSWORD=brainego_password
...
MINIO_DEFAULT_BUCKETS=brainego
EOF
```

You can keep this exact file (defaults) and only change values when ports or credentials must be customized locally.

### One-command startup options

#### Option A (recommended for full local stack bootstrap)

```bash
./init.sh
```

#### Option B (Phase 1 compose stack only)

```bash
docker compose -f docker-compose.phase1.yaml up -d
```

Both commands are single-command startup workflows. Use Option B when you specifically want the **Phase 1 MVP baseline services only**.

### Quick health verification

```bash
docker compose -f docker-compose.phase1.yaml ps
...
Expected signals:
- MAX Serve and Qdrant return healthy responses.
- Redis returns `PONG`.
- Postgres and MinIO are listed as `running`/`healthy` in `docker compose ps`.
